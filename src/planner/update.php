...
$stmt = $pdo->prepare('SELECT id, user_id FROM ' . TBL_OUTFITS_PLANNED . ' WHERE id = ? AND user_id = ?');
...
try {
    $up = $pdo->prepare('UPDATE ' . TBL_OUTFITS_PLANNED . ' SET note = ?, season_hint = ? WHERE id = ?');
