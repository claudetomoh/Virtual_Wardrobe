...
test('rejects replayed nonce', async() => {
    ...
    const body = { user_id: 1, event: 'planner_update', payload: { message: 'replay test' }, nonce, ts };
    const token = createEmitterJwt();
    const hmac = createEmitterHmac(body);
    const resp1 = await axios.post(`${SOCKET_SERVER_URL}/emit`, body, { headers: { 'X-SOCKET-KEY': SOCKET_API_KEY, 'X-EMITTER-JWT': token, 'X-EMITTER-HMAC': hmac } });
    expect(resp1.data.success).toBe(true);

    // second attempt using same nonce must be rejected
    const resp2 = await axios.post(`${SOCKET_SERVER_URL}/emit`, body, { headers: { 'X-SOCKET-KEY': SOCKET_API_KEY, 'X-EMITTER-JWT': token, 'X-EMITTER-HMAC': hmac }, validateStatus: s => s < 600 });
    expect(resp2.status).toBe(403);
    expect(resp2.data && resp2.data.error).toBe('replay detected');

    // verify audit log contains replay_detected entry
    ...
    if (rc) {
        ...
        const found = parsed.find(e => e && e.type === 'replay_detected');
        ...
        const found = parsed.reverse().find(e => e && e.type === 'replay_detected');
        expect(found).toBeTruthy();
    }
});